...
func (p *SQLServer) CreateTable(t fizz.Table) (string, error) {
	sql := []string{}
	cols := []string{}
	var s string
	for _, c := range t.Columns {
		if c.Primary {
			...
			case "integer":
				s = fmt.Sprintf("\"%s\" NOT NULL IDENTITY (1,1) PRIMARY KEY", c.Name)
			default:
...
func (p *SQLServer) DropTable(t fizz.Table) (string, error) {
	return fmt.Sprintf("DROP TABLE \"%s\";", t.Name), nil
}

func (p *SQLServer) RenameTable(t []fizz.Table) (string, error) {
	if len(t) < 2 {
...
func (p *SQLServer) AddColumn(t fizz.Table) (string, error) {
	if len(t.Columns) == 0 {
...
func (p *SQLServer) DropColumn(t fizz.Table) (string, error) {
	if len(t.Columns) == 0 {
		return "", errors.New("Not enough columns supplied!")
	}
	c := t.Columns[0]
...
func (p *SQLServer) RenameColumn(t fizz.Table) (string, error) {
	if len(t.Columns) < 2 {
		return "", errors.New("Not enough columns supplied!")
	}
	oc := t.Columns[0]
...
func (p *SQLServer) AddIndex(t fizz.Table) (string, error) {
	if len(t.Indexes) == 0 {
...
func (p *SQLServer) DropIndex(t fizz.Table) (string, error) {
	if len(t.Indexes) == 0 {
		return "", errors.New("Not enough indexes supplied!")
	}
	i := t.Indexes[0]
...
func (p *SQLServer) RenameIndex(t fizz.Table) (string, error) {
	ix := t.Indexes
	if len(ix) < 2 {
		return "", errors.New("Not enough indexes supplied!")
	}
...
func (p *SQLServer) buildColumn(c fizz.Column) string {
...
func (p *SQLServer) colType(c fizz.Column) string {
	switch c.ColType {
	case "string":
		s := "255"
