...
func (p *Mssql) CreateTable(t fizz.Table) (string, error) {
	sql := []string{}
	cols := []string{}
	var s string
...
func (p *Mssql) DropTable(t fizz.Table) (string, error) {
	return fmt.Sprintf("DROP TABLE \"%s\";", t.Name), nil
}

func (p *Mssql) RenameTable(t []fizz.Table) (string, error) {
	if len(t) < 2 {
...
func (p *Mssql) AddColumn(t fizz.Table) (string, error) {
	if len(t.Columns) == 0 {
...
func (p *Mssql) DropColumn(t fizz.Table) (string, error) {
	if len(t.Columns) == 0 {
		return "", errors.New("Not enough columns supplied!")
	}
	c := t.Columns[0]
...
func (p *Mssql) RenameColumn(t fizz.Table) (string, error) {
	if len(t.Columns) < 2 {
		return "", errors.New("Not enough columns supplied!")
	}
	oc := t.Columns[0]
...
func (p *Mssql) AddIndex(t fizz.Table) (string, error) {
	if len(t.Indexes) == 0 {
...
func (p *Mssql) DropIndex(t fizz.Table) (string, error) {
	if len(t.Indexes) == 0 {
		return "", errors.New("Not enough indexes supplied!")
	}
	i := t.Indexes[0]
...
func (p *Mssql) RenameIndex(t fizz.Table) (string, error) {
	ix := t.Indexes
	if len(ix) < 2 {
		return "", errors.New("Not enough indexes supplied!")
	}
...
func (p *Mssql) buildColumn(c fizz.Column) string {
...
func (p *Mssql) colType(c fizz.Column) string {
	switch c.ColType {
	case "string":
		s := "255"
